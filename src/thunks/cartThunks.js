...
// export const updateCartItemAlternatively = (cartID, id, quantity, token) => dispatch => {
//   // starting address update
//   dispatch(cartActions.cart.set.loading);
...
//           res => {
//             // successful
//             dispatch(cartActions.cart.update.item(res));
//           }
//         );
// }
...
export const addToCart = (id, token, productID) => (dispatch, getState) => {
  ...
  if (token) {
    if (cartItem) {
      dispatch(updateCartItem(cart[cartItem].id, id, (cart[cartItem].quantity + 1), token));
    } else {
      request('/me/carts/', getConfig(
                token,
                {
    ...
    if (cartItem) {
      dispatch(cartActions.cart.update.item({id: cart[cartItem].id, quantity: (cart[cartItem].quantity + 1)}));
    } else {
